...
"반복적인 병목을 자동화로 제거하고, AI 도구를 팀이 함께 쓸 수 있는 형태로 만드는 엔지니어입니다."

- **AI 개발 워크플로를 팀에 정착시키는 일**을 해왔습니다. 개인 작업에 머물던 에이전트·스킬을 팀이 공유해 쓰는 형태로 묶어 배포하고, 프로젝트 컨텍스트 문서를 표준화해 팀이 같은 맥락으로 AI를 쓰도록 만들었습니다.
...
- **AI 응용과 팀 개발 생산성**(에이전트·도구·워크플로) 영역을 목표로 하고 있습니다.
...
- **AI 도구 팀 공유 저장소 구성** — 레포마다 흩어져 있던 개인 스킬·에이전트를 사내 GitHub Enterprise에 플러그인 형태로 모아 TF 구성원이 함께 쓰도록 공유
    - 성격별 3개 묶음으로 구성(일감·사내 문서 조회 / 디자인 시안 명세 추출 / PR 리뷰). 클론 없이 설치 커맨드만으로 쓰도록 온보딩 문서 작성
    - 구성원 전원에게 push 권한을 열고 기여 규칙을 문서화 — 도구를 고쳐서 push하면 사용하는 사람들에게 자동 반영되는 구조
- **코드 리뷰 에이전트 활용** — 리뷰 에이전트와 검증 에이전트 2단 구조로 오탐을 걸러내고, PR 규모에 따라 모델을 등급별로 라우팅해 리뷰 깊이와 토큰 비용을 함께 관리
- **팀 AI 컨텍스트 표준화** — 프로젝트 규칙 문서를 *항상 로드* / *상황별 로드* 로 분리해 컨텍스트 비용을 관리하고, 기획서·회의록을 마크다운으로 구조화해 Agent가 기능 명세를 직접 참조하는 워크플로우 정착
